---
layout: default
title: Artifact Contracts
parent: Documentation
nav_order: 1
has_children: true
permalink: /docs/artifact-contracts/
---

# Artifact Contracts

Canonical and profile-level machine contracts for authority, delegation, evidence, decisions, registry state, identifiers, and artifact families.

## In this section

- [A2A-bound trust artifacts](a2a-bound-trust-artifacts.md)
- [AIS-1 Schema Profile (Experimental)](ais1-schema-profile.md)
- [Artifact Taxonomy](artifact-taxonomy.md)
- [Authority Boundary TSMM Profile](authority-boundary-tsmm-profile.md)
- [Decision Receipts](decision-receipts.md)
- [Delegated authority assurance flow](delegated-authority-assurance-flow.md)
- [Delegation Lineage Profile](delegation-lineage-profile.md)
- [Evidence Bundle Integrity Profile](evidence-bundle-integrity-profile.md)
- [Stable identifier governance](identifier-governance.md)
- [OASF artifact family](oasf-artifact-family.md)
- [ODRL artifact family](odrl-artifact-family.md)
- [Open Trust Artifact Model](open-trust-artifact-model.md)
- [Registry Publication Profile](registry-publication-profile.md)
- [Status and Revocation Evidence](status-and-revocation-evidence.md)
