...
If you are consuming TIS as part of the **Trust Systems Modelling Stack (TSMS)**, start with [`docs/tsms.md`]({% link docs/tsms.md %}). It defines the TSMM → TIS → TGA authority boundary, candidate compatibility baseline, fail-safe version behavior, and the initial golden-path portable contracts.
